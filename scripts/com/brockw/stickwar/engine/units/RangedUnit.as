package com.brockw.stickwar.engine.units
{
          ...
          public class RangedUnit extends Unit
          {
                    ...
                    public function angleToTargetW(param1:Unit, param2:Number, param3:Number) : Number
                    {
                              ...
                              param2 = this._projectileVelocity;
                              var _loc4_:Number = StickWar.GRAVITY;
                              var _loc5_:Number = Math.sqrt((param1.px - this.px) * (param1.px - this.px));
                              _loc5_ += this.aimXOffset * Util.sgn(mc.scaleX);
                              var _loc6_:Number = param1.pheight - this.pheight;
                              _loc6_ = 0;
                              var _loc7_:Number = -(param1.pz - pz) + _loc6_ + this.aimYOffset;
                              param3 = -Math.PI / 180 * this.angleToBowSpace(param3);
                              var _loc8_:Number = param2 * param2 * Util.sin(param3) * Util.sin(param3) + 2 * _loc4_ * -_loc7_;
                              if(_loc8_ < 0)
                              {
                                        _loc8_ = 0;
                              }
                              var _loc9_:Number = (param2 * Util.sin(param3) + Math.sqrt(_loc8_)) / _loc4_;
                              if(Math.abs(_loc9_) < 0.001)
                              {
                                        return (param1.py - py) / 5;
                              }
                              return (param1.py - py) / _loc9_;
                    }
